...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model):
        ...
        self.checkpoint_interval = config['checkpoint_interval'] or 1
        self.checkpoint_run_id = config['checkpoint_run_id']
        if self.checkpoint_run_id is None:
            self.checkpoint_run_id = '{}-{}-{}'.format(
    ...
    def _load_checkpoint(self, checkpoint_path):
        # weights_only=False: our checkpoint dict holds numpy scalars (config/scheduler
        # state); PyTorch 2.6+ defaults weights_only=True and would refuse to load it.
        ...
        self.logger.info('Loaded checkpoint: {}. Resuming from epoch {}.'.format(
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if saved and (epoch_idx + 1) % self.checkpoint_interval == 0:
