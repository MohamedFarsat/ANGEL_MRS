...
class TrainDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, shuffle=False):
        ...
        # ========== [HARD-NEG] content-guided hard negative sampling — state & config ==========
        # Not in original MENTOR. Negatives are drawn from the positive item's kNN
        # lookalikes (mm_adj) with probability current_hard_neg_ratio(), else random.
        self.current_epoch = 0
        self.hard_neg_ratio = config['hard_neg_ratio'] if config['hard_neg_ratio'] is not None else 0.0
        self.hard_neg_warmup_epochs = config['hard_neg_warmup_epochs'] if config['hard_neg_warmup_epochs'] is not None else 3
        self.hard_neg_ramp_epochs = config['hard_neg_ramp_epochs'] if config['hard_neg_ramp_epochs'] is not None else 5
        # Loaded lazily on first use: mm_adj_<k>.pt is written by the MODEL constructor,
        # which runs after this dataloader is built — eager loading here would silently
        # disable hard negatives on any fresh dataset.
    ...
    # ========== [HARD-NEG] curriculum: random -> ramp -> target ratio ==========
    ...
    def _get_neg_sample(self):
        ...
        # [MULTI-NEG] neg_rows has shape [num_negatives, batch]; row 0 preserves original behaviour
    ...
    def _sample_neg_ids(self, u_ids, pos_iids=None):
        # [MULTI-NEG] returns [num_negatives, batch]; each row is an independent draw
        if pos_iids is None:
    ...
    def _sample_hard_negative(self, pos_iid, user_history):
        """Return a content-similar negative for pos_iid, or None to fall back to random.

        None is returned when: hard negatives are off / still in warmup, the coin flip
        chooses a random negative, pos_iid has no usable lookalikes, or every lookalike
        is in the user's history (false-negative guard).
        """
        ratio = self.current_hard_neg_ratio()
